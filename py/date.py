...
def calculate_day_percentage():
    ...
    if year == 2044:
        # 2044 特例：目標倒數至 6/6
        if current_date.date() <= date(2044, 1, 3):
            # 1/1 ~ 1/3 顯示剩餘 22 週
            weeks_left = 22
        else:
            # 1/4 開始為 ISO 第一週，以此類推遞減
            weeks_left = max(0, 22 - week_of_year)
    else:
        # 2044 年以前（一般情況）
        if iso_year < year:
            # 情況：如 2040-01-01，ISO 算在 2039 年第 52 週
...
# 輸出結果
print(f"今天是今年的第 {day_of_year} 天、第 {week_of_year} 週，剩下 {days_left} 天、剩 {weeks_left} 週、剩約 {estimated_workdays:.0f} 天。")
